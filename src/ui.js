...
export const UI = {
    renderDashboard: (container) => {
        ...
        const recentTasks = Storage.getTasks()
    ...
    createTaskListItem: (task) => {
        ...
        const displayStatus = isOverdue ? 'Overdue' : task.status.replaceAll('_', ' ');
    ...
    createTaskCard: (task) => {
        ...
        return `
        <div class="task-card" draggable="true" ondragstart="window.dragStart(event, '${task.id}')" onclick="window.editTask('${task.id}')" style="background: var(--bg-base); padding: var(--space-3); border-radius: var(--radius-md); border: 1px solid ${borderColor}; .task-card:active {
    cursor: grabbing;
}">
    ...
    openTaskModal: (taskId = null) => {
        let task = taskId
            ? Storage.getTasks().find(t => t.id === taskId)
            : { title: '', description: '', status: 'todo', deadline: '' };
        ...
        <div class="modal-content" style="background: var(--bg-surface); width: 90%; max-width: 500px; margin: 10vh auto; border-radius: var(--radius-lg); padding: var(--space-6); box-shadow: var(--shadow-lg); border: 1px solid var(--border-strong);">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--space-4);">
                ...
                <button class="btn-icon" onclick="window.closeModal()" 
                    style="background: transparent; border: none; color: var(--text-muted); cursor: pointer; font-size: 1.5rem;">
                    &times;
                </button>
            </div>

            <input type="text" id="taskTitle" value="${task.title}" placeholder="Task Title"
                style="width: 100%; padding: var(--space-3); margin-bottom: var(--space-4); background: var(--bg-base); border: 1px solid var(--border-subtle); color: var(--text-primary); border-radius: var(--radius-md); font-size: var(--font-size-base); outline: none;" autofocus>

            <textarea id="taskDesc" placeholder="Description..."
                style="width: 100%; padding: var(--space-3); margin-bottom: var(--space-4); background: var(--bg-base); border: 1px solid var(--border-subtle); color: var(--text-primary); border-radius: var(--radius-md); min-height: 100px; resize: vertical; font-family: var(--font-family-body); outline: none;">
                ${task.description}
            </textarea>
            ...
            <input type="date" id="taskDeadline" value="${task.deadline || ''}"
                style="width: 100%; padding: var(--space-3); margin-bottom: var(--space-4); background: var(--bg-base); border: 1px solid var(--border-subtle); color: var(--text-primary); border-radius: var(--radius-md); outline: none;">
            ...
            <select id="taskStatus"
                style="width: 100%; padding: var(--space-3); margin-bottom: var(--space-6); background: var(--bg-base); border: 1px solid var(--border-subtle); color: var(--text-primary); border-radius: var(--radius-md); outline: none;">
